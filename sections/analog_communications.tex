...
che si semplifica\footnote{
    La soluzione di questo integrale non esiste, tuttavia possiamo provare ad assegnargli un valore sfruttando la teoria delle distribuzioni (funzioni generalizzate). Possiamo notare che:
    \[
        x(t) = \int_{-\infty}^{t} \cos(2\pi f_m \tau) d\tau = \cos(2\pi f_m t) \ast u(t)
    \]
    Quindi la sua TCF (generalizzata) è:
    \[
...
\[ A_c e^{j m_f \sin(2\pi f_m t)} = \sum_{k=-\infty}^\infty A_c J_k(m_f) e^{j k 2\pi f_m t} \]
