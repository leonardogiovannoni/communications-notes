...
\[
    R_s = \frac{1}{T_s} = \frac{1}{mT_b} = \frac{R_b}{m} = \frac{R_b}{\log_2 M}
\]
...
\]
dove $g(t) = g_T(t) \ast h(t) \ast g_R(t)$ e $n(t) = w(t) \ast g_R(t)$. 
Si può dimostrare che l'autocorrelazione per un sistema come questo è:
\[
    R_n(\tau) = R_w(\tau) \ast g_R(\tau) \ast g_R(-\tau) 
\]
Per cui la densità spettrale di potenza del rumore diventa:
\[
    S_n(f) = S_w(f) |G_R(f)|^2
\]
...
Un approccio non perseguibile consiste nel trasmettere impulsi di durata finita e quindi con banda illimitata. Questo è in contrasto con la limitatezza messa a disposizione dal canale di trasmissione \( (B_c < \infty) \), questo implica che gli impulsi \( g_T(t) \) devono avere durata infinita.
...
\[
    E_s = P_s T_s = \frac{A}{2} = \frac{M^2 - 1}{6} \quad \text{energia media per simbolo in banda passante}
\]
...
Può essere trovata ricorsivamente, per un certo numero di bit \texttt{m}, col seguente codice:
